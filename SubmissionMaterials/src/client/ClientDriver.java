...
//                System.out.println("Registration successful. Game is full and has started.");
    ...
    public void getResponse() throws RemoteException {
        ...
        // Task class to take input from user to implement timeout
        ...
        Task task = new Task();
        try {
            // Timeout mechanism for the user input. If no response by 15 minutes, continue with default
            ExecutorService executor = Executors.newSingleThreadExecutor();
            Future<String> result = executor.submit(task);
            result.get(responseTime, TimeUnit.SECONDS);
            executor.shutdown();
        } catch (Exception e) {
    ...
    /**
     * Notifies the server the game is starting
     */
